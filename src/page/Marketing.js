...
export default class MarketingScreen extends Component{
    ...
    getMarketing = async()=>{
        ...
        axios.get('http://hris.omindtech.id/api/employee-sum',{
            ...
            res = res.data;
            let temp_data = [];
            temp_data = res.data.filter(v=> v.divisi.id_role === 6)
            .map(
                v => {
                    return{
                        id: v.id,
        ...
        if(this.state.isLoading){
            ...
            return(
                <View style={styles.container}>
                    ...
                    <Text style={styles.textNavigation} onPress={()=>this.props.navigation.navigate('Division')}>Marketing Division</Text>
                    {/* EMPloyee */}                
                    <FlatList
                        style={{top: 63, left: 25}}
                        contentContainerStyle={{paddingBottom: 95}}
                        showsVerticalScrollIndicator={false}
                        data={this.state.marketing}
