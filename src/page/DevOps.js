...
export default class DevOpsScreen extends Component{
    constructor(props){
        super(props)
        this.state = { 
            devOps: [],
    ...
    getDevOps = async()=>{
        ...
        axios.get('http://hris.omindtech.id/api/employee-sum',{
            ...
            res = res.data;
            let temp_data = [];
            temp_data = res.data.filter(v=> v.divisi.id_role === 4)
            .map(
                v => {
                    return{
                        id: v.id,
        ...
        if(this.state.isLoading){
            ...
            return(
                <View style={styles.container}>
                    ...
                    <Text style={styles.textNavigation} onPress={()=>this.props.navigation.navigate('Division')}>Dev Ops Division</Text>
                    {/* EMPloyee */}                
                    <FlatList
                        style={{top: 63, left: 25}}
                        contentContainerStyle={{paddingBottom: 95}}
                        showsVerticalScrollIndicator={false}
                        data={this.state.devOps}
