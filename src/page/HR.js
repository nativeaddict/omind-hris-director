...
export default class HRScreen extends Component{
    constructor(props){
        super(props)
        this.state = { 
            hr: [],
    ...
    getHR = async()=>{
        ...
        axios.get('http://hris.omindtech.id/api/employee-sum',{
            ...
            res = res.data;
            let temp_data = [];
            temp_data = res.data.filter(v=> v.divisi.id_role === 1)
            .map(
                v => {
                    return{
                        id: v.id,
        ...
        if(this.state.isLoading){
            ...
            return(
                <View style={styles.container}>
                    ...
                    <Text style={styles.textNavigation} onPress={()=>this.props.navigation.navigate('Division')}>Human Resource Division</Text>
                    {/* EMPloyee */}                
                    <FlatList
