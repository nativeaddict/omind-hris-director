...
export default class ProductionScreen extends Component{
    ...
    getProduction = async()=>{
        ...
        axios.get('http://hris.omindtech.id/api/employee-sum',{
            ...
            res = res.data;
            let temp_data = [];
            temp_data = res.data.filter(v=> v.divisi.id_role === 3)
            .map(
                v => {
                    return{
                        id: v.id,
        ...
        if(this.state.isLoading){
            ...
            return(
                <View style={styles.container}>
                    ...
                    <Text style={styles.textNavigation} onPress={()=>this.props.navigation.navigate('Division')}>Production Division</Text>
                    {/* EMPloyee */}                
                    <FlatList
